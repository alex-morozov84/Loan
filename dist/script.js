...
window.addEventListener('DOMContentLoaded', function () {
  ...
  new _modules_playVideo__WEBPACK_IMPORTED_MODULE_2__["default"]('.module__video-item .play', '.overlay').init(); // другой вариант создания экзмемпляра и вызова метдода

  new _modules_difference__WEBPACK_IMPORTED_MODULE_3__["default"]('.officerold', '.officernew', '.officer__card-item').init();
  new _modules_form__WEBPACK_IMPORTED_MODULE_4__["default"]('.form').init();
  new _modules_accordion__WEBPACK_IMPORTED_MODULE_5__["default"]('.module__info-show').init();
    ...
    value: function bindTrigger() {
      this.btns.forEach(function (btn) {
        btn.addEventListener('click', function () {
          ...
          content.style.display = 'block'; // content.classList.add('animated', 'fadeInDown');
      ...
      try {
        container.querySelector('.plus').addEventListener('click', function () {
          ...
          if (counter !== items.length - 2) {
            ...
            counter++; // Когда доходим до последней карточки, то открываем ее и убираем кнопку
          } else {
            items[counter].style.display = 'flex';
            items[counter].classList.add('animated', 'fadeIn');
  ...
  _createClass(Form, [{
    ...
    value: function initMask() {
      var setCursorPosition = function setCursorPosition(pos, elem) {
        ...
        elem.focus(); // перемещает каретку после +7, если поместили в самое начало (чтоб нельзя было удалить +7)
      ...
      function createMask(event) {
        ...
        def = matrix.replace(/\D/g, ''),
            val = this.value.replace(/\D/g, ''); // отменяем возможность удалить +7 (т.е. когда количество введенных символов меньше количества начальных, в строку вставляется начальное значение)
...
function () {
  function VideoPlayer(triggers, overlay) {
    ...
    this.close = this.overlay.querySelector('.close'); // при создании нового экземпляра класса YT.Player мы создаем реакцию на событие 'onStateChange'. К этому событию привязываем метод this.onPlayerStateChange. Но! Т.к. он привязан к экземпляру нового объекта, то контекст теряется. Поэтому мы привязываем его к нужному контексту императивно
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn, i) {
        try {
          ...
          var blockedElem = btn.closest('.module__video-item').nextElementSibling; // каждый второй элемент (если без остатка делится на 2)
        ...
        btn.addEventListener('click', function () {
          ...
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            ...
            _this.activeBtn = btn; // если плеер уже был создан ранее, то прото показываем модалку. Если не был, то создаем новый
            ...
            if (document.querySelector('iframe#frame')) {
              ...
              _this.path = btn.getAttribute('data-url'); // запускаем плеер
      ...
      this.close.addEventListener('click', function () {
        ...
        _this.player.stopVideo(); // мой вариант решения проблемы с неоднократным включением плеера. Здесь при закрытии окна блок с iframe снова заменяется на первоначальный div
    ...
    value: function onPlayerStateChange(state) {
      try {
        // получаем блок с закрытым видео
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; // получаем картинку с треугольником (play), чтоб поставить ее вместо замка. Без параметра true будет скопирована только нода (без всего, что лежит внутри)

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // если видео закончено (номер состояний указаны в документации)

        if (state.data === 0) {
          // если класс closed существует,то мы его удаляем (Если до этого видео уже просмотрели, то класса уже не будет)
          if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
            blockedElem.querySelector('.play__circle').classList.remove('closed'); // удаляем иконку с замочком

            blockedElem.querySelector('svg').remove(); // вставляем иконку play
    ...
    value: function init() {
      ...
      if (this.btns.length > 0) {
        ...
        tag.src = "https://www.youtube.com/iframe_api"; // помещаем созданный тэг перед самым первым скриптовым тэгом
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      try {
        ...
        if (n == 3) {
          ...
          setTimeout(function () {
            _this.hiddenBlock.style.opacity = '1';

            _this.hiddenBlock.classList.add('slideInUp');
          }, 3000);
        } else {
    ...
    value: function bindTriggers() {
      ...
      this.prevBtns.forEach(function (item) {
      ...
      this.nextBtns.forEach(function (item) {
        item.addEventListener('click', function (e) {
          e.stopPropagation();
          e.preventDefault();
...
function (_Slider) {
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      this.slides.forEach(function (slide) {
        slide.classList.remove(_this.activeClass); // если передан параметр animated, то задаем нужным элементам прозрачность
      ...
      this.slides[0].classList.add(this.activeClass); // если передан параметр animated, то задаем нужным элементам прозрачность
    ...
    value: function nextSlide() {
      ...
      this.prev.addEventListener('mouseenter', function () {
        return clearInterval(_this2.autoChangeSlide);
      });
    ...
    value: function autoPlay() {
      ...
      this.autoChangeSlide = setInterval(function () {
    ...
    value: function init() {
      ...
      try {
        ...
        var autoChangeSlide;
        this.bindTriggers();
        this.decorizeSlides(); // Если параметр autoplay передан, то включается автопереключение слайдов

        if (this.autoplay) {
          this.autoPlay(); // запуск автопереключения слайдов при покидании курсора кнопок и слайдов

          this.next.addEventListener('mouseleave', function () {
            return _this5.autoPlay();
          });
...
var Slider = // в первом варианте (для одного слайдера) передавали в конструктор аргументы
...
function Slider() {
  ...
  this.container = document.querySelector(container); // обернули в try/catch для защиты от ошибок. В остальных свойствах ошибок не будет. При отсутствии параметров их результатами будет просто null. А в данном случае возникнет ошибка, т.к. children от null не существует
